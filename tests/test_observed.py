...
def test_observed_ut1_single_batch_and_complete_state(ctx):
    ...
    rows=ctx.observed.batch_at_ut1([taiyin.Body.sun,taiyin.Body.moon],jd,flags=flags)
    ...
    assert ctx.observed.at_ut1(taiyin.Body.sun,jd,flags=flags).apparent.longitudeRadians==rows[0].apparent.longitudeRadians

def test_observed_utc_and_horizontal_refraction(ctx):
    utc=taiyin.AstroDateTime(2024,4,8,18); assert len(ctx.observed.batch_at_utc([taiyin.Body.sun,taiyin.Body.moon],utc,flags=(taiyin.ObservedFlag.truePosition,)))==2
    ...
    row=ctx.observed.at_ut1(taiyin.Body.sun,taiyin.JulianDate.from_double(2460409.0),flags=(taiyin.ObservedFlag.speed,taiyin.ObservedFlag.topocentric,taiyin.ObservedFlag.horizontal,taiyin.ObservedFlag.refraction,taiyin.ObservedFlag.truePosition))
...
def test_observed_validation_and_close(ctx):
    ...
    assert ctx.observed.batch_at_ut1([],jd)==[]

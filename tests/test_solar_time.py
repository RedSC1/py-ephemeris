...
def test_calculates_equation_of_time_from_ut1_and_tt(ctx):
    ...
    assert math.isclose(from_tt.value.equationSeconds, equation.equationSeconds, abs_tol=2e-5)
    ...
    assert from_ut1.diagnostic.status == 0
    assert from_tt.diagnostic.status == 0
...
def test_equation_of_time_cpp_multi_epoch_oracles(ctx, ut1, expected_seconds):
    ...
    assert math.isclose(result.value.equationSeconds, expected_seconds, abs_tol=2.0)


def test_round_trips_local_mean_and_apparent_solar_time(ctx):
    ut1 = taiyin.JulianDate.from_double(2460311.0)
    longitude_radians = 116.3833 * math.pi / 180.0
    equation = ctx.solar_time.equation_of_time_at_ut1(ut1).value
    ...
    round_trip = ctx.solar_time.apparent_to_mean(apparent.value)

    assert abs(apparent.value.coordinate.seconds_difference(local_mean.coordinate) - equation.equationSeconds) < 1e-4
    assert abs(round_trip.value.coordinate.seconds_difference(local_mean.coordinate)) < 1e-4
    assert round_trip.value.longitudeRadians == longitude_radians
    assert apparent.diagnostic.status == 0
    assert round_trip.diagnostic.status == 0

...
Event searches take an interval or an estimate and return typed result objects.
Keep the requested step size appropriate for the body and event; the API can
provide recommended longitude/aspect steps.
...
twilight = ctx.visibility.solar_twilight_at_ut1(
    start,
    end,

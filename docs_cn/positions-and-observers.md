...
| API 类别 | 原生计算期间的 GIL | 并发边界 |
|---|---|---|
| 位置、状态、恒星、观测坐标 | 释放 | 已配置 context 可共享执行只读计算。 |
| 事件、可见性、轨道、掩星、日月食、晨昏首次可见 | 释放 | 可共享 context；provider/cache 竞争仍可能限制加速。 |
| 岁差、恒星黄道、分宫、月交点与月球远地点 | 释放 | 内置模型可并行；Python callback 会重新取得 GIL。 |
| 中国历法、干支、八字重计算 | 释放 | 只读计算可并行；不要并发修改或关闭对象。 |
| 紫微本命盘与流运构建 | 释放 | 不同 chart 可并行；同一 chart 的流运栈不能并发修改。 |
| runtime/catalog 修改、回调注册、配置与清理 | 保留或串行化 | worker 启动前完成。 |
| 极短的值转换和纯查表 helper | 通常保留 | 原生工作太短，释放 GIL 反而没有收益。 |
